...
(function ($) {
    ...
    var rtsJs = {
        ...
        sideMenu:function(){
          // Auto-update mobile menu from desktop menu dynamically
          var $mobileMenu = $('#mobile-menu-active');
          if ($mobileMenu.length) {
              $mobileMenu.empty();
              var $desktopItems = $('.navigation__menu > ul > li').clone();
              $desktopItems.each(function() {
                  var $li = $(this);
                  $li.removeClass('navigation__menu--item');
                  if ($li.hasClass('has-child')) {
                      $li.removeClass('has-child has-arrow').addClass('has-droupdown');
                  }
                  $li.find('a').each(function() {
                      var $a = $(this);
                      if ($a.parent().hasClass('has-droupdown') || $a.parent().hasClass('has-child')) {
                          $a.removeClass('navigation__menu--item__link').addClass('main');
                          $a.attr('href', 'javascript:void(0);');
                      } else {
                          $a.removeClass('navigation__menu--item__link').addClass('mobile-menu-link');
                      }
                  });
                  $li.find('ul').each(function() {
                      var $ul = $(this);
                      $ul.removeClass('submenu sub__style').addClass('submenu mm-collapse');
                      $ul.parent('li').removeClass('has-child has-arrow').addClass('has-droupdown');
                  });
              });
